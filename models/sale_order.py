...
class SaleOrder(models.Model):
    ...
    def generate_xml_file(self):
        ...
        date_val.text = str(self.date_order)
        payment_term = ET.SubElement(doc, "PaymentTermId")
        payment_term.text = str(self.payment_term_id.name)

        template_data = ET.SubElement(doc, "QuotationTemplate")
        template_data.text = str(self.sale_order_template_id.name)
        ...
        amount_untaxed.text = str(self.amount_untaxed)
        amount_tax = ET.SubElement(doc, "Taxes")
        amount_tax.text = str(self.amount_tax)
        amt_total = ET.SubElement(doc, "Total")
        amt_total.text = str(self.amount_total)

        xmlstr = ET.tostring(root)
        file = base64.encodestring(xmlstr)
        self.file = file
        self.file_name = 'Sale Order'+str(self.name) + '.xml'
        action = {
            'type': 'ir.actions.act_url',
            'name': 'contract',
            'url': '/web/content/sale.order/%s/file/Sales Order.xml?download=true' % (
                self.id),
            'target': 'self',
        }
        return action
